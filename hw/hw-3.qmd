...
Exercises 8 and 9 use the `county` dataset in the **openintro** package.
    You can find out more about the dataset by inspecting its documentation with `?county` and you can also find this information [here](https://openintrostat.github.io/usdata/reference/county.html).
...
b\.
Which of the following two plots makes it easier to compare poverty levels (`poverty`) across people from different median education levels (`median_edu`)?
...
Note that wherever a categorical variable is used in the plot, it's `metro`. Lay them out in the same way using the chunk opton `layout-ncol: 2`.
